...
const License = ({ isWizard = false }) => {
  ...
  useEffect(() => {
    const fetchLicenseInfo = async () => {
      try {
        ...
        const response = await axios.post(USPIN_CONFIG_ADMIN?.ajax_url, new URLSearchParams({
          action: 'uspw_get_license_info',
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(USPIN_CONFIG_ADMIN?.ajax_url, new URLSearchParams({
        action: 'uspw_save_license_info',
        nonce: USPIN_CONFIG_ADMIN.nonce,
        uspw_pro_license_key: inputLicenseKey,
        uspw_pro_license_email: email,
        who: 'license_tab'
      }));
       ...
       */
      localStorage.setItem('uspw_setup_wizard_step', 1);

      Swal.fire({
        icon: 'success',
        title: __('Success', 'ultimate-spin-wheel'),
      ...
      Swal.fire({
        ...
        showConfirmButton: true,
        html: error?.response?.data?.data?.message || 'Please try again'
      });
  ...
  const deactivateLicense = async () => {
    try {
      ...
      const response = await axios.post(USPIN_CONFIG_ADMIN?.ajax_url, new URLSearchParams({
        action: 'uspw_deactivate_license',
        nonce: USPIN_CONFIG_ADMIN.nonce,
        who: 'license_tab'
      }));
        ...
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2">
          <div>
            {!licenseStatus && (
              <form onSubmit={handleAuthSubmit}>
                <div className="flex items-center gap-6">
                  <div className="w-[80%]">
                    <h6 className="mb-2 text-slate-800 text-lg font-semibold dark:text-white">
                      {__('Activate your Premium Plugin', 'ultimate-spin-wheel')}
                    </h6>
                    <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">
                      {__('Without a valid license key, you won\'t be able to access the premium features.', 'sky - elementor - addons')}
                    </p>
                  </div>
                </div>
                ...
                >
                  {__('Activate', 'ultimate-spin-wheel')}
                </button>
              </form>
              ...
              <div className="relative">
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                  <caption className="pb-5 text-lg font-semibold text-left rtl:text-right text-gray-900 dark:text-white">
                    {__('License Information', 'ultimate-spin-wheel')}
                    <p className="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                      {__('This is your current license information.', 'ultimate-spin-wheel')}
                    </p>
                  </caption>
                  <tbody>
                ...
                >
                  {__('Deactivate', 'ultimate-spin-wheel')}
                </button>
              </div>
            )}
          </div>
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
              {__('How to get License Key', 'ultimate-spin-wheel')}
            </h3>
