...
const Notice = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const response = await axios.get(USPIN_CONFIG_ADMIN?.rest_url + 'smartcart/v1/notices', {
          params: { who: 'dashboardApp' },
          headers: { 'X-WP-Nonce': USPIN_CONFIG_ADMIN.nonce }
        });
        const fetchedNotices = response?.data?.data?.notices || [];
        setNotices(fetchedNotices);
      } catch (error) {
        console.error('Error fetching notices.', error);
        setError(__('Error fetching notices.', 'ultimate-spin-wheel'));
      }
    };
  ...
  const handleDismiss = (index) => {
    ...
    const response = axios.get(USPIN_CONFIG_ADMIN?.rest_url + 'smartcart/v1/notices', {
